class Vendedores {
  ...
  public set valorDaVenda(valorDaVenda: number) {
    if (valorDaVenda <= 0) {
      console.log('Valor de vendas incorreto');
      return;
    }
    this._valorDaVenda = valorDaVenda;
  }
...
class VendedorPessoaJuridica extends Vendedores {
  ...
  public set totalFuncionarios(totalFuncionarios: number) {
    if (totalFuncionarios === 0) {
      console.log('Total de funcionarios incorreto')
      return;
    }
...
console.log(`O Salario total pessoa juridica: ${salarioTotalJuridica} || Comissao: ${comissaoJuridica}`)